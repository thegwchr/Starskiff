...
class StatusMenuBase: NSMenu, NSMenuDelegate {
    ...
    private var driverState: itl_80211_state = ITL80211_S_INIT {
        didSet {
            /** Only allow if network card is enabled or if the network card does not load
             either due to rtw88 not loaded or just not able to receive info
            ...
            case ITL80211_S_SCAN:
                /** API does not report bgscan to Starskiff. During `ITL80211_S_RUN` the status
    ...
    let rtw88VerItem = HPMenuItem(title: .rtw88Ver)
    ...
    private func getDeviceInfo() {
        DispatchQueue.global(qos: .background).async {
            ...
            var rtw88Ver: String = .unavailable
            ...
            if get_platform_info(&platformInfo) {
                ...
                rtw88Ver = String(cCharArray: platformInfo.driver_info_str)
            }
            ...
            DispatchQueue.main.async {
                if let items = self as? StatusMenuItems {
                    ...
                    items.setValueForItem(self.rtw88VerItem, value: rtw88Ver)
                }
            }
...
private extension String {
    ...
    static let rtw88Ver = NSLocalizedString("Version: ")
    ...
    static let aboutHeliport = NSLocalizedString("About Starskiff")
    static let quitHeliport = NSLocalizedString("Quit Starskiff")
